...
```r
class(gapminder$continent) #gapminder$continent %>% class()
```
...
```r
nlevels(gapminder$continent) #gapminder$continent %>% nlevels()
```
...
```r
levels(gapminder$continent) #gapminder$continent %>% levels()
```
...
```r
str(gapminder$continent) #gapminder$continent %>% str()
```
...
```r
gapminder$country %>% levels() %>% datatable()
```
...
gap$country%>% nlevels() #still 142 levels
```
...
h_gap_dropped$country%>% nlevels() #now 5 levels
```
...
So let's first plot how life the expectancy changes over time for 5 countries of your choice:
...
```r
#gapminder %>% fct_relevel() #Reorder first and then rename it
...
gapminder %>%
  ...
  coord_flip()+
  theme_bw()+
  ylab('Number of entries') + xlab("Continents")
```
