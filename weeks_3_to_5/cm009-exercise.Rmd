...
guest_long %>% 
  separate(name, into = c('first','last'), sep=' ') %>% 
   unite(col = "name", first, last, sep = ' ') # b/t col and sep, list vars you want combined, seperated by comma
```
...
```{r}
email %>% 
  separate_rows(guest, sep = ', ') #sep should be comma AND space
```
...
experiment %>% complete(nesting(name, trt), rep) #instead of expand(), use nesting() b/c each subj had 1 trt
```
