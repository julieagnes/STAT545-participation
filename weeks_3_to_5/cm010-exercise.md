...
```r
time %>% 
  inner_join(album, by = c('song','artist_name')) #also add cols from album
```
...
```r
time %>% 
  semi_join(album, by = "song") #filtering join; not left join?
```
...
```r
#My solution
length(unique(time$artist_name));length(unique(album$artist_name))
```
...
```r
setdiff(unique(time$artist_name),unique(album$artist_name))
```
