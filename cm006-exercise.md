...
```r
#select(gapminder, lifeExp:country) #in descending order
```
...
```r
gapminder %>%
  mutate(GDP=gdpPercap*pop, gdpBill=GDP/1000000000)
```
...
```r
gapminder %>%
  transmute( gdpBill=gdpPercap*pop/1000000000) %>% round(digits=2) #only keeps newly defined var(s)
```
...
```r
#gapminder %>%
#  mutate(if(country=='Canada' & year==1952) then 70 else lifeExp))
```
